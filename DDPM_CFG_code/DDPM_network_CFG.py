...
class ConvNextBlock_condition(nn.Module):
    ...
    def forward(self, x, time_emb=None, context_emb = None):
        ...
        if exists(self.mlp) and exists(time_emb) and exists(context_emb):
            condition = self.mlp(time_emb)
            condition_c = self.mlp(context_emb)
            h = h + rearrange(condition, "b c -> b c 1 1")
            h = h + rearrange(condition_c, "b c -> b c 1 1")
...
class Unet(nn.Module):
    def __init__(
        ...
        if use_convnext:
            block_klass = partial(ConvNextBlock_condition, mult=convnext_mult)
        else:
    ...
    def forward(self, x, c, time, context_mask):
        x = self.init_conv(x)
